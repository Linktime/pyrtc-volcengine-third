...
class RealtimeDialogClient:
    ...
    async def _send(self, event: int, payload: str | bytes, headers: bytearray=None, session_id: str=None):
        if headers is None:
            headers = protocol.generate_header()
    ...
    async def connect(self) -> None:
        ...
        PYRTC_LOGGER.debug(f"StartConnection response: {protocol.parse_response(response)}")
        ...
        response = await self.ws.recv()
        PYRTC_LOGGER.debug(f"StartSession response: {protocol.parse_response(response)}")

    async def task_request(self, audio: bytes) -> None:
        task_request = protocol.generate_header(message_type=protocol.CLIENT_AUDIO_ONLY_REQUEST,
    ...
    async def chat_request(self, payload: dict) -> None:
        chat_request = protocol.generate_header(message_type=protocol.CLIENT_FULL_REQUEST,
    ...
    async def finish_connection(self):
        ...
        response = await self.ws.recv()
        PYRTC_LOGGER.debug(f"FinishConnection response: {protocol.parse_response(response)}")
